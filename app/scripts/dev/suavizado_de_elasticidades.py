...
for m in list_mat:
    if count % 1000 == 0:
        print(f"Procesando material {count} de {len(list_mat)}: {m}")
    count += 1
    for z in list_zon:
        for c in list_can:
            df_ev = df[
...
# table_id = "onus-dev-proy-retail-elastici.staging.test_suavizado_semanal"
table_id = "onus-dev-proy-retail-elastici.staging.test_suavizado_semanal_prev"
